...
export default function BookingDetails() {
  ...
  useEffect(()=>{ fetch("http://47.128.219.68:9000/events").then(r=>r.json()).then(d=>{ const e=d.events.find(ev=>ev.id.toString()===eventId); setEvent(e?{...e,remainingSeats:e.remaining_seats,image:e.banner_url?`http://47.128.219.68:9000${e.banner_url}`:eventBg}:null); setLoading(false); }).catch(()=>setLoading(false)); },[eventId]);

  const handleBooking=()=>{ if(quantity>event.remainingSeats){setMessage(`Only ${event.remainingSeats} seats available`);return;} setBooking(true); const fd=new FormData(); fd.append("quantity",quantity); fetch(`http://47.128.219.68:9000/bookings/${eventId}/${userId}`,{method:"POST",body:fd}).then(r=>r.json()).then(d=>{ if(d.detail)setMessage(d.detail); else{setSuccess(true);setEvent(p=>({...p,remainingSeats:p.remainingSeats-quantity}));setQuantity(1);setMessage("");} }).catch(()=>setMessage("Error booking event")).finally(()=>setBooking(false)); };
